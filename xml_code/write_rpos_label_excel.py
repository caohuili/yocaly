...
#from  run_result_file import lepu_r_dict
from xml_code import get_rpos_type


def write_r_excel(path,date):
	wb = openpyxl.load_workbook(os.getcwd() + '\\xml_code\\rpos_label.xlsx')
	excel_tag = ['错检r_pos', 'yocaly', 'lepu', '漏检r_pos', 'label', '多检r_pos', 'label']
	patientid_names = patientid_dict.patientid_dict
	#lepu_d=lepu_r_dict.ecg_r_dict
	#for key, value in yl_diff_dict.items():
	line_empty = 2
	for pid,key in patientid_names.items():
	#for key in lepu_d:
		yl_diff_dict, move_list = get_per_r_result(path,date,key)
		#sheet = wb.create_sheet(key, 0)
		if bool(move_list):
			value = yl_diff_dict[key]
			sheet = wb.create_sheet(key, 0)
			...
			sheet1.cell(row=line_empty, column=1).value = pid
			sheet1.cell(row=line_empty, column=2).value = key
			line_empty += 1

	wb.save(os.getcwd() + '\\run_result_file\\' + date + '_xml_result.xlsx')

def get_per_r_result(path, date,y_key):
	yl_diff_dict = {}
	print(y_key)
	y_value = get_rpos_type.get_patient_rpos(path,date,'yocaly_download',y_key)
	l_value = get_rpos_type.get_patient_rpos(path,date,'lepu_download',y_key)
	...
	if bool(l_value):
		...
		for i in range(len(more_and_less_list) - 1):
			if (more_and_less_list[i + 1] - more_and_less_list[i]) < 20:
				...
				if more_and_less_list[i] in more_r_list:
					...
					if more_and_less_list[i + 1] in yuser_r_dict:
						pass
					elif luser_r_dict[more_and_less_list[i + 1]] == yuser_r_dict[more_and_less_list[i]]:
						pass
					else:
...
def get_r_result(y_key):
	...
	print(y_key)
	y_value = yocaly_rpos_dict[y_key]
